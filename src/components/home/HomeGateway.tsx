...
export interface GatewayCopy {
  ...
  research: { name: string; body: string; profiles: string; references: string };
  areas: { name: string; body: string };
  search: { name: string; label: string; placeholder: string; submit: string };
...
export interface GatewayData {
  ...
  worlds: readonly { world: WorldId; label: string; href: string; price: string | null }[];
 ...
 *
 * NOT A MEGA-MENU. Each door shows the thing it opens onto: the catalogue by
 * a real product and its lowest price, the research by the literature it
 * cites, the areas by their eight signs in their own hues, the worlds by their
 * grounds. Every figure is counted from a registry.
 *
    ...
    <section className={styles.gateway} aria-labelledby="gateway-title">
      <Container width="full">
        ...
        <div className={styles.board} data-doors={quality ? 6 : 5}>
          ...
          {/* 2 — THE RESEARCH, shown by what it cites. */}
          <Link href={data.links.research} className={`${styles.door} ${styles.research}`}>
            ...
            <span className={styles.figures}>
              <span className={styles.figure}>
                <span className={styles.figureValue}>{pad(counts.profiles)}</span>
                <span className={styles.figureLabel}>{copy.research.profiles}</span>
              </span>
              <span className={styles.figure}>
                <span className={styles.figureValue}>{pad(counts.references)}</span>
                <span className={styles.figureLabel}>{copy.research.references}</span>
              </span>
            </span>
            {data.recentReferences.length > 0 ? (
              <span className={styles.bibliography} aria-hidden="true">
                {data.recentReferences.map((ref) => (
          ...
          {/* 3 — THE AREAS, shown by their eight signs. */}
          <Link
            href={data.links.areas}
            className={`${styles.door} ${styles.small} ${styles.areas}`}
          >
            <span className={styles.doorName}>{copy.areas.name}</span>
            <span className={styles.signs} aria-hidden="true">
              {data.areas.map((area) => (
                <span key={area.id} className={styles.sign} data-area={area.id} title={area.label}>
                  <AreaIcon id={area.id} className={styles.signIcon} />
                </span>
              ))}
            </span>
            <span className={styles.doorBody}>{fill(copy.areas.body, { n: counts.areas })}</span>
            <span className={styles.go} aria-hidden="true">
              →
            </span>
          </Link>
          ...
          {/* 5 — THE SIGNATURE WORLDS, shown by their grounds. */}
